...
import { AllCourseRatesResponseType, CourseRateType } from "@/app/Redux/types";
...
const CourseRates = ({
  ...
  reviews: CourseRateType[];
  refetch: () => Promise<{
    data?: AllCourseRatesResponseType;
    error?: unknown;
  }>;
}) => {
  ...
  const handleChangeCourseStatus = async (CourseId: number, status: number) => {
    try {
      ...
      showSuccess(`${t("alerts.rate_status_changed_success")}`);
      await refetch();
    } catch {
      showError(`${t("alerts.rate_status_changed_failed")}`);
    }
  };
